...
> This article was written in collaboration with [Chris Kenst](https://kenst.com), another software testing professional, who provided assistance with some of the structure and organization of this post.
>
...
>
> Go check out his work as well! Direct link to his post mirroring this one can be found [here](https://www.kenst.com/unit-testing-your-test-framework/).
...
2. We generate a list of similar tests we want to run. To do this we have:
   ...
   2. A `predicate`, which is a function we create to make a decision as to whether the values we received from the website are correct or not, which we set to a `util.isSortedLowToHi{:js}` function
...
Now if we `npm run unit{:bash}`... Aha! We have observed failing results. We now have data that we know for sure is causing a problem.
